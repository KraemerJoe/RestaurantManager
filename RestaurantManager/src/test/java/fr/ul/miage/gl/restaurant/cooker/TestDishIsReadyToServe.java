...
public class TestDishIsReadyToServe {

	@Test
	@DisplayName("Ensure a dish is set as ready to be serve")
	public void testDishIsReadyToBeServed() {
		...
		MockiEbean.runWithMock(mock, () -> {
			...
			for (SessionOrder sessionOrder : ordersSession) {
				assertEquals(sessionOrder.getStatut(), EnumSessionOrderStat.READY_TO_SERVE);
			}
		});
	}
}
