	...
	public void executeChoice(int choice) {
		...
		case 4:
			...
			while (!finish) {
				...
				if (materials.size() <= rawId || materials.get(rawId) == null) {
					...
					int quantity = MenuUtil.askForPositiveInt(
							"Enter the quantity of " + rawChoosed.getName() + " needed in the dish.");
			...
			if (!error) {
				...
				for (CompositionDish compositionDish : compositions) {
					System.out.println(" > " + compositionDish.getRawMaterial().getName() + " | x"
							+ compositionDish.getQuantity());
				}
		...
		case 5:
			...
			for (PreparationTimeDish preparationTimeDish : preparations) {
				...
				String timed = String.format("%dmn %ds", TimeUnit.MILLISECONDS.toMinutes(divided),
